...
d3.chart.dependencyWheel = function(options) {
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      var fill = function(d) {
        var pN = packageNames[d.index];
        var packageColor = 0;
        for (var i = 0; i < 7; i++){
          packageColor += pN.substr(i, 1).charCodeAt()
        }
        packageColor += pN.substr(3, 1).charCodeAt()
        //console.log(packageColor);
        if (d.index === 0) return '#ccc';
        return "hsl(" + parseInt(((packageColor % 95) / 26) * 360, 10) + ",90%,70%)";
      };
